...
while (!(entrada = sc.nextLine()).equals("FIM")) 
{
    try 
    {
        ...
        Pokemon encontrado = ProcurarPokemon.selecao(selecionados, pokemonName);
...
class ProcurarPokemon 
{
    ...
    public static Pokemon selecao(List<Pokemon> pokemons, String name)
    {
        Pokemon foundPokemon = null;
        for (int i = 0; i < pokemons.size(); i++) 
        { 
            // Pegar o pokemon da lista na posição i
            Pokemon pokemon = pokemons.get(i); 
            if (pokemon.getName().equals(name)) 
            { 
                foundPokemon = pokemon; // Armazenar o Pokémon encontrado
                i = pokemons.size(); // Interromper o loop, já que o Pokémon foi encontrado
            }
        }
        return foundPokemon;
}
