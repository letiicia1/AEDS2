...
void ordenar(Pokemon* pokemons, int esq, int dir) 
{
    int i = esq, j = dir;
    int pivo = pokemons[(esq + dir) / 2].generation;
    char pivo2[100];  
    strcpy(pivo2, pokemons[(esq + dir) / 2].name);  // Copia o nome do pivô

    while (i <= j) {
        // Comparar se generation é menor que o pivô ou se o nome é menor que o pivô
        while (pokemons[i].generation < pivo || 
              (pokemons[i].generation == pivo && strcmp(pokemons[i].name, pivo2) < 0)) {
            i++;
            comparacoes++;
        }

        // Comparar se generation é maior que o pivô ou se o nome é maior que o pivô
        while (pokemons[j].generation > pivo || 
              (pokemons[j].generation == pivo && strcmp(pokemons[j].name, pivo2) > 0)) {
            j--;
            comparacoes++;
        }
...
int main(void) {
    ...
    FILE* log = fopen("840757_quicksort.txt", "w");
    if (log) {
