...
//classe principal
public class OrdenacaoQuicksortParcial
{
   ...
   while (!(entrada = sc.nextLine()).equals("FIM")) 
   {
       ...
       catch (NumberFormatException e) 
       {
           ...
           // insercao os Pokémon
           ordenacao.ordenar(selecionados, 0, selecionados.size() - 1);
           ...
           String logFileName = "840757_quicksortparcial.txt";
...
class Ordenacao
{
    ...
    public void ordenar(List<Pokemon> pokemons, int esq, int dir) 
    {
        int i = esq; int j = dir;
        int pivo = pokemons.get((esq + dir) / 2).getGeneration();
        String pivo2 =  pokemons.get((esq + dir) / 2).getName();
    
        while (i <= j) {
            // Comparar se generation é menor que o pivô ou se o nome é menor que o pivô
            while (pokemons.get(i).getGeneration()< pivo || 
                  (pokemons.get(i).getGeneration() == pivo && pokemons.get(i).getName().compareTo(pivo2) < 0)) {
                i++;
                numComparacoes++;
            }
    
            // Comparar se generation é maior que o pivô ou se o nome é maior que o pivô
            while (pokemons.get(j).getGeneration() > pivo || 
            (pokemons.get(j).getGeneration() == pivo && pokemons.get(j).getName().compareTo(pivo2) > 0)) 
            {
                j--;
                numComparacoes++;
            }
