...
public class OrdenacaoInsercao
{
   ...
   while (!(entrada = sc.nextLine()).equals("FIM")) 
   {
       ...
       catch (NumberFormatException e) 
       {
           ...
           // insercao os Pokémon
           ordenacao.insercao(selecionados);
...
class Ordenacao
{
    ...
    public void insercao(List<Pokemon> pokemon) {
        for (int i = 1; i < pokemon.size(); i++) {
            Pokemon tmp = pokemon.get(i);
    
            int j = i - 1;
    
            // Usa compareTo para comparar as datas e, se forem iguais, compara os nomes
            while (j >= 0 && (pokemon.get(j).getCaptureDate().compareTo(tmp.getCaptureDate()) > 0 || 
                    (pokemon.get(j).getCaptureDate().compareTo(tmp.getCaptureDate()) == 0 && 
                    pokemon.get(j).getName().compareTo(tmp.getName()) > 0))) {
    
                        numComparacoes++;

                        pokemon.set(j + 1, pokemon.get(j));  
                        numMovimentacoes++;
