...
// Função que realiza o algoritmo de Insercao Parcial

void ordenar(Pokemon* pokemon, int n) 
{
        for (int i = 1; i < n; i++) 
        {
           ...
           int date1 = tmp.captureDate.ano *10000 + tmp.captureDate.mes *100 + tmp.captureDate.dia;
            int date2 = pokemon[j].captureDate.ano * 10000 + pokemon[j].captureDate.mes * 100 + pokemon[j].captureDate.dia;
            
            while (j >= 0 && (date2 > date1 || (date1 == date2 && strcmp(pokemon[j].name, tmp.name) > 0))) 
            {
                ...
                j--;
                if (j >= 0) {
                date2 = pokemon[j].captureDate.ano *10000  + pokemon[j].captureDate.mes *100 + pokemon[j].captureDate.dia;
                }
            }
...
clock_t inicio = clock(); // Captura o tempo inicial
    ...
    FILE* log = fopen("840757_insercaoparcial.txt", "w");
    if (log) {
