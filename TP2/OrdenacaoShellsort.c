...
void insercaoPorCor(int cor, int h, Pokemon* array,int n) {
...
// Função que realiza o algoritmo Shellsort

void ordenar(Pokemon* pokemon, int i) {
    int h = 1; 

    // Gera a sequência de intervalos de Shellsort
    do {
        h = (h * 3) + 1; 
    } while (h < i); 

    // Realiza a ordenação usando a sequência de intervalos
    do {
        h /= 3; 

        // Para cada "cor" (coluna), realiza a inserção
        for (int cor = 0; cor < h; cor++) {
            insercaoPorCor(cor, h, pokemon, i); 
        }
    } while (h > 1); 
}
...
clock_t inicio = clock(); // Captura o tempo inicial
    ...
    FILE* log = fopen("840757_shellsort.txt", "w");
    if (log) {
