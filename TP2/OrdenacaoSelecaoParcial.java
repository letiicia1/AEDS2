...
while (!(entrada = sc.nextLine()).equals("FIM")) 
{
    ...
    catch (NumberFormatException e) 
    {
        ...
        // Ordenar os Pokémon
        ordenacao.ordenar(selecionados);
        ...
        String logFileName = "840757_selecaoParcial.txt";
...
class Ordenacao {
    ...
    public void ordenar(List<Pokemon> pokemons) {
        for (int i = 0; i < 10; i++) {
            int temp = i;
            for (int j = i + 1; j < pokemons.size(); j++) {
                // Comparar os nomes dos Pokémons
                numComparacoes++;
                if (pokemons.get(temp).getName().compareTo(pokemons.get(j).getName()) > 0) {
                    temp = j;
                }
            }
            // Trocar os Pokémons de lugar
            if (temp != i) {
                Pokemon temp1 = pokemons.get(i);
                pokemons.set(i, pokemons.get(temp));
                pokemons.set(temp, temp1);
                numMovimentacoes = 3 + numMovimentacoes;
            }
        }
    }
