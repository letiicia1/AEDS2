...
public class OrdenacaoCountingSort {
    ...
    public static void main(String[] args) {
        ...
        ordenacao.countingSort(selecionados);
        ...
        String logFileName = "840757_countingsort.txt";
...
class Ordenacao {
    ...
    public void countingSort(List<Pokemon> pokemons) {
        // Obter o maior valor de captureRate entre os pokemons
        int maxCaptureRate = getMaior(pokemons);
        ...
        for (int i = 0; i <= maxCaptureRate; count[i] = 0, i++);
        ...
        // Modificar o array de contagem para armazenar as posições finais
        for (int i = 1; i <= maxCaptureRate; count[i] += count[i - 1], i++);

        // Inicializar a lista ordenada com valores nulos
        for (int i = 0; i < pokemons.size(); i++) {
            ordenado.add(null);
        }

        // Ordenar os Pokemons pelo captureRate
        for (int i = pokemons.size() - 1; i >= 0; i--) {
            Pokemon pokemon = pokemons.get(i);
            int captureRate = pokemon.getCaptureRate();
            int index = count[captureRate] - 1;
            ordenado.set(index, pokemon);
            count[captureRate]--;
            numMovimentacoes++;  
        }

        // Atualizar a lista original com a lista ordenada
        for (int i = 0; i < pokemons.size(); i++) {
            pokemons.set(i, ordenado.get(i));
        }

        // Ordenar por nome no caso de empate no captureRate
        desempatePorNome(pokemons);
    }

    // Função auxiliar para encontrar o maior captureRate
    private int getMaior(List<Pokemon> pokemons) {
    ...
    // Função para desempatar pelo nome em caso de empate no captureRate
    private void desempatePorNome(List<Pokemon> pokemons) {
    // Laço para ordenar todas as seções onde há empate no captureRate
    int i = 0;
    while (i < pokemons.size() - 1) {
        int j = i;
        // Verifica todos os próximos Pokémons com a mesma taxa de captura
        while (j < pokemons.size() - 1 && pokemons.get(j).getCaptureRate() == pokemons.get(j + 1).getCaptureRate()) {
            j++;
        }
        ...
        for (int k = i; k < j; k++) {
            for (int l = i; l < j - (k - i); l++) {
                numComparacoes++; 
                if (pokemons.get(l).getName().compareTo(pokemons.get(l + 1).getName()) > 0) {
                    Pokemon temp = pokemons.get(l);
                    pokemons.set(l, pokemons.get(l + 1));
                    pokemons.set(l + 1, temp);
                    numMovimentacoes++; 
                }
            }
        }
