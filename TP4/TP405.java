...
class Hash {
    ...
    public boolean inserir(Pokemon elemento) {
       boolean resp = false;
       if (elemento != null && elemento.getName() != null) {
    ...
    public int pesquisar(String elemento) {
       ...
       if (elemento != null) {
          ...
          int pos = h(num);
          TP405.comparisonCount++;
          if (tabela[pos] != null && tabela[pos].getName().equals(elemento)) {
             posicao = pos;
          } else {
             for (int i = 0; i < reserva; i++) {
                TP405.comparisonCount++;
                if (tabela[m1 + i] != null && tabela[m1 + i].getName().equals(elemento)) {
                   posicao = m1 + i;
                   break;
                }
             }
