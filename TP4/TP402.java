...
public class TP402 
{
 ...
 public static void main(String[] args) throws Exception 
 {
     Scanner sc = new Scanner(System.in);
        ...
        ArvoreArvore selecionados = new ArvoreArvore();
...
while (!(entrada = sc.nextLine()).equals("FIM")) 
{
    try 
    {
       ...
       Pokemon encontrado =  ProcurarPokemon.procurar( pokemons, pokemonName);
        System.out.println("=> " + pokemonName);
        System.out.print("raiz ");

        if (selecionados.pesquisar(encontrado)) 
        {
...
//Classe da arvore 
 class ArvoreArvore {
   ...
   public void inserir(Pokemon s, No i) throws Exception {
		if (i == null) {
         throw new Exception("Erro ao inserir: captureRate invalido!");

      } else if ( (s.getCaptureRate() % 15) < i.elemento) {
         inserir(s, i.esq);
	...
	private No2 inserir(Pokemon s, No2 i) throws Exception {
		if (i == null) {
         i = new No2(s);
	...
	public boolean pesquisar(Pokemon elemento) {
		return pesquisar(raiz, elemento);
	}

	private boolean pesquisar(No no, Pokemon x) {
        boolean encontrado = false;
    
        
        if (no != null) {
           
            encontrado = pesquisarSegundaArvore(no.outro, x);
    
          
            if (!encontrado) {
                System.out.print("ESQ ");
                encontrado = pesquisar(no.esq, x);
            }
    
            if (!encontrado) {
                System.out.print("DIR ");
                encontrado = pesquisar(no.dir, x);
            }
        }
	...
	private boolean pesquisarSegundaArvore(No2 no, Pokemon x) {
      boolean resp;
		if (no == null) { 
         resp = false;

      } else if (x.getName().compareTo(no.elemento.getName()) < 0) {
        TP402.comparisonCount++;
         System.out.print("esq ");
...
class ProcurarPokemon 
{
    ...
    public static Pokemon procurar(List<Pokemon> pokemons, String id)
    {
        Pokemon foundPokemon = null;
